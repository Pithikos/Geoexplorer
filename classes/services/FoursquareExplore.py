...
#
# MAX PROBLEM:
# Scan (59.625, 16.5, 59.59375, 16.625) for "restaurant".
# In the area there are two MAX restaurants. Both are logged to the
# results log file. However only one of the two has a marker on the map.
# The one lacking a marker is the one just right of Pilgatan street.
# Furthermore, as the results count variable on the Scanner depends on
# the amount of markers, we get one less count than what we have in
